...
barplot <- ggplot(comb_genus_passed, aes(x = station, fill = class_name, y = Freq)) +
  geom_bar(position = "stack", stat = "identity", color = "black", size = 0.0) +
  labs(
  ...
  guides(fill = guide_legend(title = "Family"))  # Customize legend title
