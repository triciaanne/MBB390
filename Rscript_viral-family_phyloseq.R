...
#gives relative abundance of viruses according to family
viruses_physeq_norm = transform_sample_counts(viruses_physeq, function(x) x / sum(x) )
p <- plot_bar(viruses_physeq_norm, fill = "Family")
p + geom_bar(aes(fill=Family), stat="identity", position="stack") + scale_fill_manual(values=MyPalette)